...
public class TableroBichos {
	...
	public TableroBichos(final int f, final int c, final int anchoBicho, final int altoBicho,
			final VentanaJuegoTablero v) {
		this.filas = f;
		this.columnas = c;
		...
		for (int f = 0; f < this.filas; ++f) {
			for (int c = 0; c < this.columnas; ++c) {
				if ((f == 0 /*|| f == 1 || f == this.getFilas() - 1*/) && (c == 9)) {
					...
					final Transparencia caram = new Transparencia(new CoordTablero(f, c), "Transparencia", anchoBicho,
							altoBicho, this);
	...
	public Bicho getObjetoDC(final CoordTablero ct) {
		...
		if (ct.getFila() < -1 || ct.getColumna() < 0 || ct.getFila() >= this.getFilas()
				|| ct.getColumna() >= this.getColumnas()) {
			return null;
	...
	public void intercambiaCaramelos(final CoordTablero origen, final CoordTablero destino) {
		...
		this.tablero[destino.getFila()][destino.getColumna()]
				.setPosicionTablero(this.tablero[origen.getFila()][origen.getColumna()].getPosicionTablero());
