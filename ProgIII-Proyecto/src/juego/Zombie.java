...
public abstract class Zombie extends Bicho implements Movible {
	...
	public boolean mover() {
		...
		if (this.tablero.getObjetoDC(caida) != null) {
			...
			if (this.tablero.getObjetoDC(caida) instanceof Bala) {
				this.vida -= 50;
				if (this.vida <= 0) {
					this.quitar();
				}
		 ...
		 * false; } else if (this.tablero.getObjetoDC(caida) instanceof Planta)
		 * { this.tablero.getObjetoDC(caida).setVida(this.tablero.getObjetoDC(
		 * caida).getVida()-25); if
		 * (this.tablero.getObjetoDC(caida).getVida()==0) {
		 * this.tablero.mueveZombie(this.posicion, caida); } else { return
